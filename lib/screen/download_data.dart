...
class _DownloadDataState extends State<DownloadData>{
  ...
  List<String> items = ["Table_Structure", "Journey_Plan","Non_Working_Reason", "Brand_Master", "Sku_Master",
    "Sub_Category_Master", "Category_Master"];
  ...
  _fetchData(var index) async {
    ...
    final url =
        "http://juul.parinaam.in/webservice/juul.svc/DownloadAll";
     ...
     await http.post(url, body: lData, headers: lHeaders).then((response) {
        ...
        switch(items[index]){
          ...
          case "Journey_Plan":
            ...
            //
            _insertData(response.body, "Journey_Plan");
          ...
          case "Brand_Master":
            _insertData(response.body, "Brand_Master");
            break;

          case "Sku_Master":
            _insertData(response.body, "Sku_Master");
            break;

          case "Sub_Category_Master":
            _insertData(response.body, "Sub_Category_Master");
            break;

            case "Category_Master":
            _insertData(response.body, "Category_Master");
            break;
        }
